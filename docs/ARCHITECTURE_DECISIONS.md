...
## ADR — AI Platform closure is repository-owned and testable

`BIL-AI-025-R1` makes the Product Owner-approved closure sequence explicit in code and living documents. Each capability has a stable key, delivery state, non-empty testable exit criteria, and an execution successor. The plan is immutable and offline-only; it is governance data, not a scientific or recommendation engine.
