...
| Android health minimization | The 2026-09-16 remediation source requests only Health Connect reads for steps, distance and active calories burned. No writes or extended-history/background permissions. BLE remains limited to standard weight-scale, body-composition and heart-rate fitness profiles. Final AAB and Console still require matching evidence. |
| Apple health minimization | HealthKit permission text is localized; the current platform-specific write scope is body mass only. This Android remediation does not modify the Apple bridge. |
