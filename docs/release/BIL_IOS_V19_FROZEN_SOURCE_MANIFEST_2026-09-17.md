# BIL iOS 1.0.0 build 19 — unified release source manifest
...
`RELEASE_BUILD_NUMBER: 19`

## Release identity and scope

iOS build 19 is the next candidate after build 18. It includes the unified
Apple purchase ownership, refund, trial, restore and entitlement safeguards;
the current dashboard polish and accessibility work; and the reviewed
premium-content and connected-health access boundaries.

The signed iOS workflow supplies `--build-number 19`, which becomes
`CFBundleVersion 19` while the public version remains `1.0.0`. Its manual
dispatch defaults to uploading only the validated, signed IPA to TestFlight;
the upload step remains downstream of all source, signing and App Store Connect
validation gates.

The binding is not self-referential: after this final unified source commit is
pushed, `BIL_IOS_V19_AUDITED_SOURCE_SHA` must equal that exact commit and
`BIL_IOS_V19_STAGING_MANIFEST_SHA256` must equal this file's committed-byte
...
- iOS 1.0.0, CFBundleVersion 19.
- Android 1.0.0, versionCode 16.
