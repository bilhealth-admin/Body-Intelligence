...
BIL processes nutrition logs, body measurements, activity, preferences and any
health data the user explicitly chooses to connect. Data is used to provide
logging, trends, reports, synchronization and requested features. Sensitive
health data is not sold and is not used for advertising. Local-first features
remain on the device unless the user enables an authenticated cloud feature.
Cloud processing uses encrypted transport and access controls. Users may revoke
...
BIL is a wellness and self-tracking tool. It does not diagnose, treat, cure or
prevent disease and does not replace a qualified healthcare professional.
Users remain responsible for entries, device connections and decisions made
from their information. Paid plans renew through the platform store under the
price and period shown by that store; cancellation and refunds follow Apple or
Google rules. Community content must follow the published content policy and
...
Authenticated users can delete eligible cloud records and community content
...
Free has no store purchase. Plus and Pro monthly or annual access begins only
after the platform store and BIL entitlement boundary verify the transaction.
The store displays the local price, currency, tax treatment, renewal period,
and cancellation terms. BIL does not apply a fixed global VAT rate. Manage or
cancel through Apple or Google; Restore Purchases is available in the app.
Trials are not advertised unless an active store configuration proves one.
...
BIL supports activity and fitness, nutrition and weight management, sleep and
general wellness tracking. It does not claim regulated medical-device status.
Users should consult a qualified professional for medical advice, diagnosis or
treatment. Connected-health permissions are requested only for user-selected
features and may be revoked from the operating system.
