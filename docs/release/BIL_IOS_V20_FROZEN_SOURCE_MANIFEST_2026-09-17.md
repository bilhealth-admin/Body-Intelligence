# BIL iOS 1.0.0 build 20 — unified release source manifest
...
iOS build 20 is the next candidate after build 19. It includes the unified
...
The signed iOS workflow supplies `--build-number 20`, which becomes
`CFBundleVersion 20` while the public version remains `1.0.0`. Its manual
...
pushed, `BIL_IOS_V20_AUDITED_SOURCE_SHA` must equal that exact commit and
`BIL_IOS_V20_STAGING_MANIFEST_SHA256` must equal this file's committed-byte
