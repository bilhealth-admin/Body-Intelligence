# BIL iOS 1.0.0 build 16 — release source manifest
...
This corrective candidate follows the uploaded build 15 candidate commit
`0760d48f5b3acc592d93c8d4ffd4526ad7b33b46`.
...
the signed workflow overrides it with `--build-number 16` / `CFBundleVersion 16`.

The binding is not self-referential: after the commit is pushed,
`BIL_IOS_V16_AUDITED_SOURCE_SHA` must equal that exact commit and
`BIL_IOS_V16_STAGING_MANIFEST_SHA256` must equal this file's committed-byte
