# BIL Android 1.0.0 build 16 — unified release source manifest
...
`RELEASE_BUILD_NUMBER: 16`

## Release identity and source boundary

Android build 16 is the next candidate after build 15 and uses the same final
unified source as iOS build 19. It includes the reviewed Health Connect scope,
Apple/Google purchase entitlement safeguards shared by the app, and the current
dashboard, accessibility and premium-content corrections.

The Android release workflow overrides the shared Flutter version with
`--build-number 16`, producing `versionCode 16` while `versionName` remains
`1.0.0`.

The binding is not self-referential: after this final unified source commit is
pushed, `BIL_ANDROID_V16_AUDITED_SOURCE_SHA` must equal that exact commit and
`BIL_ANDROID_V16_STAGING_MANIFEST_SHA256` must equal this file's committed-byte
...
a physical device. Before Play review, the Play Console Health Apps declaration,
Data Safety, reviewer access, subscription/trial metadata and public listing
must match this binary. Signed CI and device/store testing remain distinct
evidence.
