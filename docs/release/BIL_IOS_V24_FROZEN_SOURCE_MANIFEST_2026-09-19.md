# BIL iOS 1.0.0 build 24 — unified release source manifest
...
iOS build 24 is the next candidate after build 23, which is already present in
...
The signed iOS workflow supplies `--build-number 24`, which becomes
`CFBundleVersion 24` while the public version remains `1.0.0`. Its manual
...
pushed, `BIL_IOS_V24_AUDITED_SOURCE_SHA` must equal that exact commit and
`BIL_IOS_V24_STAGING_MANIFEST_SHA256` must equal this file's committed-byte
