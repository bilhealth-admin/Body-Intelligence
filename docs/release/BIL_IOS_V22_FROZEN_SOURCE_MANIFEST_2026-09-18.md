# BIL iOS 1.0.0 build 22 — unified release source manifest
...
iOS build 22 is the next candidate after build 21. It includes the unified
...
The signed iOS workflow supplies `--build-number 22`, which becomes
`CFBundleVersion 22` while the public version remains `1.0.0`. Its manual
...
pushed, `BIL_IOS_V22_AUDITED_SOURCE_SHA` must equal that exact commit and
`BIL_IOS_V22_STAGING_MANIFEST_SHA256` must equal this file's committed-byte
