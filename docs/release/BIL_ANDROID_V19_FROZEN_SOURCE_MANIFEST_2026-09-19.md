# BIL Android 1.0.0 build 19 — unified release source manifest
...
Android build 19 is the next candidate after build 18 and uses the same final
unified source as iOS build 24. It includes the reviewed Health Connect scope,
Apple/Google purchase entitlement safeguards shared by the app, the dashboard,
accessibility, premium-content, native-permission, Food Log, and AI Coach
corrections.

The Android release workflow overrides the shared Flutter version with
`--build-number 19`, producing `versionCode 19` while `versionName` remains
`1.0.0`.

The binding is not self-referential: after this final unified source commit is
pushed, `BIL_ANDROID_V19_AUDITED_SOURCE_SHA` must equal that exact commit and
`BIL_ANDROID_V19_STAGING_MANIFEST_SHA256` must equal this file's committed-byte
