...
class BILGlobalHealthBridge(
    ...
    // A Dart deadline cannot cancel an in-flight Health Connect read. Keep the
    // native operation single-flight so a second tap cannot overlap a large
    // history import while the first coroutine is still draining.
    private val readChangesInFlight = AtomicBoolean(false)
    private var readChangesJob: Job? = null
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "readChanges" -> {
                if (!readChangesInFlight.compareAndSet(false, true)) {
                    result.error(
                        "operation_in_progress",
                        "A Health Connect sync is already in progress.",
                        mapOf("platform" to "health_connect"),
                    )
                    return
                }
                readChangesJob = scope.launch {
                    try {
                        result.success(withContext(Dispatchers.IO) { readChanges(call) })
                    } catch (cancelled: CancellationException) {
                        result.error("cancelled", cancelled.message, mapOf("platform" to "health_connect"))
                    } catch (security: SecurityException) {
                        result.error("unauthorized", security.message, mapOf("platform" to "health_connect"))
                    } catch (error: Throwable) {
                        result.error("health_connect_failure", error.message, mapOf("type" to error::class.java.simpleName))
                    } finally {
                        readChangesJob = null
                        readChangesInFlight.set(false)
                    }
                }
            }
            "cancelReadChanges" -> {
                readChangesJob?.cancel(CancellationException("Health Connect sync cancelled."))
                result.success(null)
            }
