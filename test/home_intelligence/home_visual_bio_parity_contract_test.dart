...
void main() {
  test('primary cards reuse Bio Intelligence twin deck dimensions', () {
    final primary = File(
      'lib/features/dashboard/widgets/dashboard_primary_carousel.dart',
    ).readAsStringSync();

    expect(primary, contains('DashboardTwinDeckShell('));
    expect(primary, contains("title: arabic ? 'أفضل خطوة الآن'"));
    expect(primary, contains("title: arabic ? 'ملخص اليوم'"));
    expect(primary, contains("title: arabic ? 'رؤى اليوم'"));
    ...
    expect(primary, isNot(contains('SingleChildScrollView(')));
  ...
  test('phone dashboard preserves all original sections below top deck', () {
    final benchmark = File(
      'lib/features/dashboard/widgets/premium_dashboard_benchmark.dart',
    ).readAsStringSync();

    expect(benchmark, contains('phonePrimaryCarousel'));
    expect(benchmark, contains('primaryActionPage'));
    expect(benchmark, contains('primaryInsightsPage'));
