...
class _ChatRepository extends CommunityRepository {
  _ChatRepository() : super(SupabaseClient('https://chat-unit.invalid', 'unit-test-key',
  ...
  @override
  Future<List<CommunityMessage>> loadMessages(String id) => pendingLoad?.future ?? Future.value([
    CommunityMessage(id: 'message-1', senderId: id, recipientId: currentUserId,
      body: 'Previously loaded message', createdAt: DateTime(2026, 9, 8, 10, 30)),
  ]);
  @override
  Future<void> markConversationRead(String id) async {}
  @override
  Future<void> sendMessage(String id, String body) => pendingSend?.future ?? Future.value();
}
Widget _chat(_ChatRepository repository) => MaterialApp(home: CommunityChatPage(
  userId: '22222222-2222-4222-8222-222222222222', displayName: 'QA chat', repository: repository));
void main() {
  testWidgets('realtime refresh keeps selectable history visible while loading', (tester) async {
  ...
  testWidgets('a new draft typed during send is not erased by the old completion', (tester) async {
    ...
    await tester.pumpAndSettle();
    expect(tester.widget<TextField>(composer).controller!.text, 'Next unsent draft');
    await tester.pumpWidget(const SizedBox.shrink());
    expect(tester.takeException(), isNull);
  });
  testWidgets('leaving during send does not write to a disposed controller', (tester) async {
    final repository = _ChatRepository()..pendingSend = Completer<void>();
    addTearDown(repository.changes.close);
    await tester.pumpWidget(_chat(repository));
    await tester.pumpAndSettle();
