...
class _Repository extends CommunityRepository {
  _Repository() : super(SupabaseClient('https://unit-test.invalid', 'unit-test-key',
  ...
  @override
  Future<List<Map<String, dynamic>>> loadMyFoodSubmissions() async { foodLoads++; return []; }
  @override
  Future<void> publishPost(String body) async { textCalls++; }
  @override
  Future<void> publishPostWithImage(String body, CommunityPostImageDraft image) async { imageCalls++; }
}
...
void main() {
  testWidgets('empty publication has a visible reason and does not send', (tester) async {
    final repository = _Repository();
    await tester.pumpWidget(MaterialApp(home: CommunityHubPage(repository: repository, postImagePicker: _Picker())));
  ...
  testWidgets('photo without caption is explained, then text plus photo sends', (tester) async {
    final repository = _Repository();
    await tester.pumpWidget(MaterialApp(home: CommunityHubPage(repository: repository, postImagePicker: _Picker())));
    await tester.pumpAndSettle();
    await tester.tap(find.byKey(const Key('community-create-post')));
    ...
    await tester.pumpAndSettle();
    expect(find.text('Write a caption before publishing your photo.'), findsOneWidget);
    ...
    await tester.pumpAndSettle();
    expect(find.text('Write a caption before publishing your photo.'), findsNothing);
    await tester.ensureVisible(publish);
    await tester.pumpAndSettle();
    expect(publish.hitTestable(), findsOneWidget);
    await tester.tap(publish);
  ...
  testWidgets('photo composer remains reachable in a short keyboard-sized viewport', (tester) async {
  ...
  testWidgets('food list is not reloaded by a parent rebuild', (tester) async {
    final repository = _Repository();
    Widget app() => MaterialApp(home: CommunityHubPage(repository: repository, postImagePicker: _Picker()));
