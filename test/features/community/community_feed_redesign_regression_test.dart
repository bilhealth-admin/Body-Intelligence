...
class _FeedRepository extends CommunityRepository {
  _FeedRepository() : super(SupabaseClient('https://unit.invalid', 'test',
  ...
  Future<List<CommunityPost>> loadFeed({int limit = 40}) async => [
    CommunityPost(id: 'post', authorId: currentUserId,
      body: 'A saved community post', createdAt: DateTime.utc(2026, 9, 8)),
  ];
  @override
  ...
  @override
  Future<void> publishPost(String body) async { calls++; }
}

void main() {
  testWidgets('feed is for reading; creation is separate and back preserves its draft', (tester) async {
    final repository = _FeedRepository();
    await tester.pumpWidget(MaterialApp(home: CommunityHubPage(repository: repository)));
    ...
    await tester.pumpAndSettle();
    expect(find.byKey(const Key('community-post-editor-page')), findsOneWidget);
    await tester.enterText(find.byKey(const Key('community-post-composer')), 'Saved draft');
