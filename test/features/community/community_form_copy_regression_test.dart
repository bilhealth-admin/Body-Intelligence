...
void main() {
  test('new form feedback covers every production locale without English fallback', () {
    ...
    for (final tag in BilLocalePolicy.productionTags) {
      for (final english in CommunityFormCopy.englishKeys) {
        ...
        if (tag != 'en') expect(text, isNot(english), reason: '$tag: $english');
        expect(communityTextForLanguage(tag, english,
          CommunityFormCopy.resolve(english, 'ar')!), text);
      }
    }
  });
}
