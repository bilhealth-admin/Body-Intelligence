...
void main() {
  ...
  test('both signed workflows execute the frozen configuration validator', () {
    ...
    expect(ios, contains('BIL_IOS_V16_AUDITED_SOURCE_SHA'));
    expect(ios, contains('BIL_IOS_V16_STAGING_MANIFEST_SHA256'));
  ...
  test('iOS validator consumes only the build 16 release manifest', () {
    final source = _read(workflows.last);
    expect(
      source,
      contains(
        'BIL_RELEASE_MANIFEST_PATH: '
        'docs/release/BIL_IOS_V16_FROZEN_SOURCE_MANIFEST_2026-09-10.md',
      ),
    );
    expect(
      source,
    ...
    expect(source, contains('(( BUILD_NUMBER == 16 ))'));
    expect(source, isNot(contains('(( BUILD_NUMBER == 15 ))')));
  });
