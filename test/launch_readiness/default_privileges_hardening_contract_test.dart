...
void main() {
  ...
  const sqlProbePath =
      'supabase/tests/default_privileges_hardening_test.sql';
  ...
  test('default-privileges migration cannot mutate current data or RLS', () {
    ...
    expect(
      RegExp(r'\brevoke\b[^;]*\bfrom\b[^;]*\bservice_role\b',
              caseSensitive: false)
          .hasMatch(migration),
      isFalse,
    ...
    expect(
      RegExp(r'^\s*grant\b[^;]*\bto\b[^;]*\b(?:anon|authenticated)\b',
  ...
  test('transactional probe creates one synthetic table and always rolls back', () {
    ...
    expect(probe, contains('create table public.bil_default_acl_hardening_probe_'));
