...
$destinationRoot = 'G:\BIL_Toolchains\UserRuntimes'
$targets = @(
  [pscustomobject]@{ Source = 'C:\Users\HP 1040 G8\AppData\Local\Microsoft\WinGet\Packages'; Name = 'winget-packages' },
  [pscustomobject]@{ Source = 'C:\Users\HP 1040 G8\AppData\Local\JxBrowser'; Name = 'jxbrowser' },
  [pscustomobject]@{ Source = 'C:\Users\HP 1040 G8\AppData\Local\Programs\Python'; Name = 'python' }
)

New-Item -ItemType Directory -Path $destinationRoot -Force | Out-Null
$resolvedRoot = (Resolve-Path -LiteralPath $destinationRoot).Path
if (-not $resolvedRoot.StartsWith('G:\BIL_Toolchains\UserRuntimes', [StringComparison]::OrdinalIgnoreCase)) {
...
foreach ($target in $targets) {
  ...
  $copiedFiles = @(Get-ChildItem -LiteralPath $staging -File -Force -Recurse)
  $copiedBytes = ($copiedFiles | Measure-Object Length -Sum).Sum
  if ($sourceFiles.Count -ne $copiedFiles.Count -or $sourceBytes -ne $copiedBytes) {
  ...
  foreach ($file in ($sourceFiles | Sort-Object Length -Descending | Select-Object -First 8)) {
    $relative = $file.FullName.Substring($source.Length).TrimStart('\')
    $copiedPath = Join-Path $staging $relative
  ...
  if ($resolvedOld -ne [IO.Path]::GetFullPath($oldSource)) {
