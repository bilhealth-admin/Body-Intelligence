...
export const subscriptionImageProducts = [
  {
...
function validateAsset(filePath) {
  ...
  if (png.bitDepth !== 8 || png.colorType !== 2) {
    throw new Error(
      ...
      } must be flattened 8-bit RGB PNG without alpha; ` +
...
async function uploadProduct(client, product) {
  ...
  if (existing.length > 0) {
    return {
      ...
      states: existing.map((image) =>
        image.attributes?.assetDeliveryState?.state ?? null
      ),
    };
  }
...
export async function main() {
  if (process.env.ASC_ALLOW_SUBSCRIPTION_IMAGE_UPLOAD !== "YES") {
    throw new Error(
      "Mutation refused: set ASC_ALLOW_SUBSCRIPTION_IMAGE_UPLOAD=YES",
    );
  }
  const client = clientFromEnvironment();
  const results = [];
  for (const product of subscriptionImageProducts) {
    results.push(await uploadProduct(client, product));
  }
  const report = {
