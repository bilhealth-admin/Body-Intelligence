...
deno.serve(async (request: Request) => {
  ...
  const providerChoice =
    (deno.env.get("BIL_MEAL_VISION_PROVIDER") ?? "legacy_gateway").trim()
      .toLowerCase();
  const directProvider =
    new Set(["openai", "gemini", "mistral"]).has(providerChoice)
      ? providerChoice as VisionProviderName
      : null;
  let directConfig: ProviderConfig | null = null;
  if (directProvider) {
    try {
      directConfig = loadProviderConfig(
        directProvider,
  ...
  if (
    ...
    (!directConfig && (!gatewayUrl || !gatewaySecret))
  ) {
    return reply({ error: "vision_not_configured" }, 503);
  }
  ...
  const requestedLocale = resolveVisionLocale(body.requested_locale);
  const digestBytes = await crypto.subtle.digest(
    "SHA-256",
    new TextEncoder().encode(
      visionCacheInput(requestedLocale, body.mime_type, body.image_base64),
    ),
  );
  ...
  if (reserveError) {
    ...
    if (message.includes("ai_boost_required")) {
      return reply({ error: "ai_boost_required" }, 402);
    }
    if (message.includes("ai_usage_exhausted")) {
      return reply({ error: "vision_quota_exhausted" }, 429);
    }
    if (message.includes("duplicate_image")) {
      return reply({ error: "duplicate_image" }, 409);
    }
    if (message.includes("idempotency_payload_mismatch")) {
      return reply({ error: "idempotency_payload_mismatch" }, 409);
    }
      ...
      if (directConfig) {
        ...
        upstream = await fetch(gatewayUrl!, {
          ...
          body: JSON.stringify({
            ...
            rules: {
              identify_visible_food_only: true,
              no_nutrition_estimation: true,
              no_medical_claims: true,
              maximum_candidates: 8,
              all_visible_text_in_requested_locale: true,
              response_locale_required: true,
              canonical_units: ["g", "ml", "piece"],
            },
          }),
        });
      }
  ...
  const safeMetric = (key: string): number | null => {
    const root = decoded && typeof decoded === "object"
  ...
  if (directConfig) {
    try {
      ...
      return reply({
        error: error instanceof VisionProviderError &&
            error.code === "language_mismatch"
          ? "vision_language_mismatch"
          : providerFailure
          ? "vision_provider_failed"
          : "invalid_vision_response",
        provider_validation_code: error instanceof VisionProviderError
          ? error.code
          : "unknown",
        provider_validation_reason: error instanceof VisionProviderError
          ? error.message
    ...
    try {
      validateVisionResponseLocale(
        root?.response_locale,
        requestedLocale,
        candidates,
      );
    } catch (_) {
      await settle(false, { latency_ms: Date.now() - startedAt });
      return reply({ error: "vision_language_mismatch" }, 502);
    }
    providerUsage = {
  ...
  if (
    providerUsage.cost_usd === null && providerUsage.input_tokens !== null &&
  ...
  const responseBody = {
    ...
    // The client owns localized review copy; legacy clients use their local
    // default notice when this field is absent rather than a forced English one.
    notice_key: "review_verified_food",
    provider_metrics: {
