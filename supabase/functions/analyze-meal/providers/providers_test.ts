...
const configs: Record<string, ProviderConfig> = {
  ...
  mistral: {
    ...
    apiKey: "test-only",
    model: "mistral-test",
    endpoint: "https://example.test/mistral",
  },
};
const modelJson = JSON.stringify({
  candidates: [{ name: "koshari", confidence: 0.9, evidence: "visible bowl" }],
});
...
Deno.test("builders keep secrets in headers and images in request bodies", () => {
  for (const config of Object.values(configs)) {
    ...
    if (request.url.includes("test-only") || !request.body.includes("AAAA")) {
      throw new Error("unsafe request construction");
    }
  }
});
...
Deno.test("all provider envelopes normalize to the BIL schema", () => {
  ...
  for (const provider of Object.keys(configs)) {
    const result = normalizeProviderResponse(
      envelopes[provider as keyof typeof envelopes],
      configs[provider],
    );
    if (
...
Deno.test("a provider markdown JSON fence is normalized without relaxing JSON parsing", () => {
  ...
  if (result.candidates[0].name !== "koshari") {
    throw new Error("fenced JSON normalization failed");
  }
});
...
Deno.test("malformed candidates fail closed", () => {
  try {
    normalizeProviderResponse({
      choices: [{
        message: { content: '{"candidates":[{"name":"","confidence":2}]}' },
      }],
    }, configs.mistral);
...
Deno.test("empty low-confidence dish identity is a safe abstention", () => {
  ...
  if (
    result.candidates[0].dish_identity !== null ||
...
Deno.test("visual artifacts and seasoning are not food components", () => {
  ...
  if (
    result.candidates[0].visible_components.length !== 1 ||
...
Deno.test("generic liquid descriptions abstain while ingredient-named sauces remain", () => {
  const content = JSON.stringify({
    candidates: [{
      ...
      visible_components: [
        ...
        {
          name: "yellow broth/sauce",
          confidence: 0.99,
          evidence: "yellow liquid",
        },
        {
          name: "blueberry sauce",
          confidence: 0.98,
          evidence: "visible berries in sauce",
        },
      ],
    }],
  });
  ...
  const names = result.candidates[0].visible_components.map((item) =>
    item.name
  );
  ...
  const content = JSON.stringify({
    candidates: [{
      ...
      visible_components: [
        ...
        {
          name: "roasted surface",
          confidence: 0.99,
          evidence: "brown exterior",
        },
      ],
    }],
  });
