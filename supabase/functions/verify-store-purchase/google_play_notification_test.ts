...
test("grace period uses the authoritative Play expiry boundary", () => {
  ...
  assert.equal(
    googleGracePeriodEnd("active", "2026-09-01T00:00:00Z"),
    undefined,
  );
...
test("parses test and subscription RTDN envelopes", () => {
  assert.deepEqual(
    parseGoogleNotification({ testNotification: { version: "1.0" } }),
    {
      kind: "test",
    },
  );
...
test("malformed or unknown RTDN fails closed", () => {
  ...
  assert.equal(
    parseGoogleNotification({ subscriptionNotification: {} }).kind,
    "invalid",
  );
  assert.equal(
    parseGoogleNotification({
