...
Deno.serve(async (request) => {
  if (request.headers.get('x-bil-dispatch-secret') !== Deno.env.get('BIL_INTERNAL_DISPATCH_SECRET')) return reply(401, {error: 'unauthorized'});
  const url = Deno.env.get('SUPABASE_URL');
  const key = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY');
  if (!url || !key) return reply(503, {error: 'cloud_not_configured'});
  const client = createClient(url, key, {auth: {persistSession: false}});
  const {data: events, error} = await client.from('bil_push_outbox').select('*').is('dispatched_at', null).order('created_at').limit(100);
  if (error) return reply(500, {error: 'outbox_read_failed'});
  let delivered = 0;
  for (const event of events ?? []) {
    const {data: tokens} = await client.from('bil_push_device_tokens').select('token_ciphertext,platform,sensitive_preview_allowed').eq('user_id', event.recipient_id).eq('enabled', true);
