...
Deno.serve(async (request) => {
  ...
  if (request.headers.get("x-bil-dispatch-secret") !== dispatchSecret) {
    ...
    await Promise.all(
      ((claimed ?? []) as ClaimedDelivery[]).map(async (token) => {
        const provider = token.platform === "apns"
          ? {
            gateway: secureGateway(Deno.env.get("BIL_APNS_GATEWAY_URL")),
            secret: Deno.env.get("BIL_APNS_GATEWAY_SECRET"),
          }
          : token.platform === "fcm"
          ? {
            gateway: secureGateway(Deno.env.get("BIL_FCM_GATEWAY_URL")),
            secret: Deno.env.get("BIL_FCM_GATEWAY_SECRET"),
          }
          : null;
