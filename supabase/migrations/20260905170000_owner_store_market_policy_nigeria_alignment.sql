...
-- Forward-only correction of the final owner-approved cross-store split.
-- Deployed migration history remains immutable:
...
set sale_enabled = false,
    ...
    classification_reason = 'not_in_owner_final_sale_market_allowlist',
    updated_at = now();
...
select code, 'token_only', 'premium',
  'owner_final_premium_only_market_2026_09_05',
  '2026-09-05-v4', true
...
select code, 'profitable', 'premium_ai_coach',
  'owner_final_all_other_sale_markets_ai_2026_09_05',
  '2026-09-05-v4', true
from unnest(array[
  ...
  'HK','HN','HR','HU','ID','IE','IL','IN','IQ','IS','IT','JM','JO','JP','KE','KG',
  'KH','KN','KR','KW','KY','KZ','LA','LB','LC','LK','LR','LT','LU','LV','LY',
  'MA','MD','ME','MG','MK','ML','MM','MN','MO','MR','MS','MT','MU','MV','MW',
  'MX','MY','MZ','NA','NE','NI','NL','NO','NP','NR','NZ','OM','PA','PE',
...
begin
  ...
  if v_code = any(array[
    ...
    'HKG','HND','HRV','HUN','IDN','IRL','ISR','IND','IRQ','ISL','ITA','JAM','JOR','JPN','KEN','KGZ',
    'KHM','KNA','KOR','KWT','CYM','KAZ','LAO','LBN','LCA','LKA','LBR','LTU','LUX','LVA','LBY',
    'MAR','MDA','MNE','MDG','MKD','MLI','MMR','MNG','MAC','MRT','MSR','MLT','MUS','MDV','MWI',
    'MEX','MYS','MOZ','NAM','NER','NIC','NLD','NOR','NPL','NRU','NZL','OMN','PAN','PER',
...
begin
  ...
  from public.bil_commerce_country_policy
  where sale_enabled = true and policy_version = '2026-09-05-v4';

  select count(*) into v_premium
  from public.bil_commerce_country_policy
  where sale_enabled = true and policy_version = '2026-09-05-v4'
    and target_plan = 'premium';

  select count(*) into v_ai
  from public.bil_commerce_country_policy
  where sale_enabled = true and policy_version = '2026-09-05-v4'
    and target_plan = 'premium_ai_coach';

  if v_enabled <> 172 or v_premium <> 4 or v_ai <> 168 then
    raise exception 'BIL market policy invariant failed: enabled=%, premium=%, ai=%',
      v_enabled, v_premium, v_ai;
  end if;

  if public.bil_market_plan_for_store_country('NG') <> 'premium'
     or public.bil_market_plan_for_store_country('NGA') <> 'premium'
     or public.bil_market_plan_for_store_country('IN') <> 'premium_ai_coach'
     or public.bil_market_plan_for_store_country('IND') <> 'premium_ai_coach'
     or public.bil_market_plan_for_store_country('CN') <> 'not_for_sale' then
    raise exception 'BIL market policy routing invariant failed';
  end if;
end
$$;
