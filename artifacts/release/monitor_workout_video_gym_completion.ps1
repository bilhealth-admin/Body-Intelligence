...
if (Test-Path -LiteralPath $monitorLock -PathType Leaf) {
    $existingMonitorId = (Get-Content -LiteralPath $monitorLock -Raw -Encoding UTF8).Trim()
    if ($existingMonitorId -match '^\d+$' -and
        $null -ne (Get-Process -Id ([int]$existingMonitorId) -ErrorAction SilentlyContinue)) {
        Write-Host "GYM_VALIDATION_MONITOR_ALREADY_RUNNING=$existingMonitorId"
        exit 0
    }
}

New-Item -ItemType Directory -Path $stateRoot -Force | Out-Null
Set-Content -LiteralPath $monitorLock -Value $PID -Encoding UTF8
