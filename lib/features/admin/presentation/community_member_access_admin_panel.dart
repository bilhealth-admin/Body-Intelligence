...
class _CommunityMemberAccessAdminPanelState
  ...
  Future<void> _confirmAndReinstate(CommunitySuspendedMemberEntry entry) async {
    ...
    try {
      ...
      if (!mounted) return;
      if (!reinstated) {
        _showFailure(StateError('community_member_reinstate_rejected'));
      } else {
        ref.invalidate(communitySuspendedMemberListProvider);
      }
    } on Object catch (error) {
  ...
  void _showFailure(Object error) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          _copy(
            _memberFailureEnglish(kind),
            ar: _memberFailureArabic(kind),
            fr: _memberFailureEnglish(kind),
            es: _memberFailureEnglish(kind),
            tr: _memberFailureEnglish(kind),
          ),
        ),
      ),
    );
  ...
  String _memberFailureArabic(AdminOperationFailureKind kind) => switch (kind) {
    AdminOperationFailureKind.timedOut =>
