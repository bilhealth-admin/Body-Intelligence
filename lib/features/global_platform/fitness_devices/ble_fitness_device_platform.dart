...
import 'fitness_device_platform.dart';

enum BleFitnessProfile { weightScale, bodyComposition, heartRate }

const Set<BleFitnessProfile> bleFitnessProfiles = <BleFitnessProfile>{
  BleFitnessProfile.weightScale,
  BleFitnessProfile.bodyComposition,
  BleFitnessProfile.heartRate,
};

const Set<String> bleFitnessMeasurementKinds = fitnessDeviceMeasurementKinds;

Set<String> _kindsFor(Set<BleFitnessProfile> profiles) => <String>{
  if (profiles.contains(BleFitnessProfile.weightScale)) 'weight',
  if (profiles.contains(BleFitnessProfile.bodyComposition)) 'body_fat',
  if (profiles.contains(BleFitnessProfile.heartRate)) 'heart_rate',
};
...
final class BlePeripheral {
  ...
  final Set<BleFitnessProfile> profiles;
}

abstract interface class BleFitnessBridge {
...
abstract interface class ManagedBleFitnessBridge implements BleFitnessBridge {
...
final class BleFitnessDeviceProvider implements FitnessDeviceProvider {
  BleFitnessDeviceProvider({
  ...
  Future<List<FitnessDeviceIdentity>> discover() async {
    ...
    for (final device in devices) {
      ...
      await store.put('fitness_device_registry', device.id, <String, Object?>{
        'name': device.name,
        'manufacturer': device.manufacturer,
        'firmwareVersion': device.firmwareVersion,
  ...
  Future<List<FitnessMeasurement>> ingest({
    ...
    for (var attempt = 1; attempt <= maxReconnectAttempts; attempt++) {
      try {
        ...
        final output = <FitnessMeasurement>[];
        for (final packet in packets) {
          final sampleId = packet['sampleId'] as String;
          final dedupKey = '$deviceId:$sampleId';
          if (await store.get('fitness_packet_seen', dedupKey) != null) {
            continue;
          }
          ...
          await store.put('fitness_packet_seen', dedupKey, <String, Object?>{
  ...
  Future<void> _updateDeviceState(
    ...
    final current =
        await store.get('fitness_device_registry', peripheral.id) ??
        <String, Object?>{};
    await store.put('fitness_device_registry', peripheral.id, <String, Object?>{
      ...current,
      'pairingState': state,
  ...
  Future<void> _auditRejected(
    ...
    GlobalAuditEvent(
      action: 'fitness.measurement.rejected',
      subjectId: deviceId,
      at: at,
      metadata: <String, Object?>{
        'sampleId': sampleId,
  ...
  double _normalize(double value, String unit, String canonicalUnit) {
    ...
    throw StateError('unsupported_fitness_unit:$unit:$canonicalUnit');
  }
